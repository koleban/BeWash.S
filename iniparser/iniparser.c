...
void iniparser_dump_array(dictionary * d, char* strings, int bufferSize)
{
    int     i ;

    if (d==NULL || strings==NULL || bufferSize < d->size*255) {printf("ini memory size error\n"); return;}
    for (i=0 ; i<d->size ; i++) {
        if (d->key[i]==NULL)
            continue ;
        if (d->val[i]!=NULL) {
            sprintf(strings+i*255, "[%s]=[%s]\n", d->key[i], d->val[i]);
        } else {
            sprintf(strings+i*255, "[%s]=UNDEF\n", d->key[i]);
        }
    }
    return ;
}
...
unsigned int iniparser_getuint(dictionary * d, const char * key, int notfound)
{
    char    *   str ;
...
unsigned long iniparser_getlong(dictionary * d, const char * key, int notfound)
{
    char    *   str ;
    str = iniparser_getstring(d, key, INI_INVALID_KEY);
    if (str==INI_INVALID_KEY) return notfound ;
    return (unsigned long)strtoul(str, NULL, 0);
}
